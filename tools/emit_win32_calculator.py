...
def emit_is_finite_ptr(e: Emitter, a: X86) -> None:
    e.label("IsFinitePtr")
    a.prologue()
    finite = e.unique("is_finite_ptr_finite")
    a.mov_reg_ebp(ECX, 8)
    a.mov_reg_ptr(EAX, ECX, 4)
    a.and_reg_imm(EAX, 0x7FF00000)
    a.cmp_reg_imm(EAX, 0x7FF00000)
    a.jcc("ne", finite)
    a.xor_reg_reg(EAX, EAX)
    a.epilogue(4)
    e.label(finite)
...
def emit_try_current_value_v2(e: Emitter, a: X86) -> None:
    ...
    a.fstp_mem64_ebp(-20)
    e.label(positive)
    finite_value = e.unique("try_current_finite")
    a.lea_reg_ebp(EAX, -20)
    a.push_reg(EAX)
    a.call("IsFinitePtr")
    a.test_reg_reg(EAX)
    a.jcc("ne", finite_value)
    a.xor_reg_reg(EAX, EAX)
    a.jmp(done)
    e.label(finite_value)
    a.mov_reg_ebp(ECX, 8)
...
def emit_format_number_v2(e: Emitter, a: X86) -> None:
    ...
    a.prologue(8)
    finite = e.unique("format_number_finite")
    a.push_ebp(8)
    a.call("IsFinitePtr")
    a.test_reg_reg(EAX)
    a.jcc("ne", finite)
